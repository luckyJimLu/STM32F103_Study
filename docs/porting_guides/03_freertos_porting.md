...
- [四、 阶段三：FreeRTOSConfig.h 核心配置裁剪](#四-阶段三freertosconfigh-核心配置裁剪)
  - [1. 基础内核运行参数宏配置](#1-基础内核运行参数宏配置)
  - [2. 内存与任务栈深度配置](#2-内存与任务栈深度配置)
  - [3. 同步与组件功能开关](#3-同步与组件功能开关)
...
### 2. 基础裸机工程准备
选择一个最简且稳定的裸机例程作为母本（推荐基础跑马灯例程或具备以下模块的工程）：
...
标准开发套件中的 `SYSTEM` 文件夹代码默认适配了裸机或 uC/OS 系统。接入 FreeRTOS 需进行针对性修改以实现平滑对接。
